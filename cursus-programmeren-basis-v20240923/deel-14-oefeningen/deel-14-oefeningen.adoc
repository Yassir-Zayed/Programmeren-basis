...
Elke __counter__ heeft een __value__ en een __step value__. 

Voorzie een `Advance` method die de __teller__ bij elke oproep met de __step value__ verhoogt. 

De _value_ van de teller is `0` bij het begin. Je kunt deze waarde instellen met `SetValue` of opvragen met de `GetValue` method.

De __step value__ is by default `1`, kan je opvragen met `GetStep`, en instellen met `SetStep`.
