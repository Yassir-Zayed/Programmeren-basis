...
De extra `Nog een keer!` output mag in de laatste herhaling niet meer verschijnen, we zullen deze dus in een __if statement__ plaatsen zodat we ze soms kunnen overslaan. De voorwaarde van die _if_ moet altijd `true` zijn, maar in de laatste herhaling moet ze `false` zijn. Dit geeft :
...
Je ziet dat de `if`-voorwaarde nu moeilijker te begrijpen is (__"Hoezo +1 ?!?"__), zeker als je de `while`-voorwaarde bekijkt. 

Daarom valt het aan te raden om de teller pas op het einde van de __loop body__ te verhogen, dat is minder verwarrend.
